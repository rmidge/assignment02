...
select
    ...
    nl.stop_name,
    case
        when nl.distance < 1000 then
            round(nl.distance::numeric) || ' meters ' ||
            case
                when nl.azimuth between 0 and pi() / 8 then 'N'
                when nl.azimuth between pi() / 8 and 3 * pi() / 8 then 'NE'
                when nl.azimuth between 3 * pi() / 8 and 5 * pi() / 8 then 'E'
                when nl.azimuth between 5 * pi() / 8 and 7 * pi() / 8 then 'SE'
                when nl.azimuth between 7 * pi() / 8 and 9 * pi() / 8 then 'S'
                when nl.azimuth between 9 * pi() / 8 and 11 * pi() / 8 then 'SW'
                when nl.azimuth between 11 * pi() / 8 and 13 * pi() / 8 then 'W'
                when nl.azimuth between 13 * pi() / 8 and 15 * pi() / 8 then 'NW'
                else 'N'
